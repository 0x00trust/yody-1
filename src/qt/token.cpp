...
bool ToHash160(const std::string& strYodyAddress, std::string& strHash160)
{
    CTxDestination yodyAddress = DecodeDestination(strYodyAddress);
    if(!IsValidDestination(yodyAddress))
        return false;
    const PKHash * keyid = boost::get<PKHash>(&yodyAddress);
    if(keyid){
...
bool ToYodyAddress(const std::string& strHash160, std::string& strYodyAddress)
{
