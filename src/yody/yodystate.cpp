...
ResultExecute YodyState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, YodyTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){
    ...
    if(voutLimit){
        ...
        return ResultExecute{
            ex,
            YodyTransactionReceipt(oldStateRoot, oldUTXORoot, gas, e.logs(), {}, {}),
        ...
        if (res.excepted == dev::eth::TransactionException::None) {
            ...
            return ResultExecute{
                res,
                YodyTransactionReceipt(rootHash(), rootHashUTXO(), startGasUsed + e.gasUsed(), e.logs(), {}, {}),
                tx ? *tx : CTransaction()
            };
        }
    }
