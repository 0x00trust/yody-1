...
#if defined(NDEBUG)
# error "Yody cannot be compiled without assertions."
#endif
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if(tx.HasCreateOrCall()){
        ...
        YodyDGP yodyDGP(globalState.get(), fGettingValuesDGP);
        uint64_t minGasPrice = yodyDGP.getMinGasPrice(::ChainActive().Tip()->nHeight + 1);
        uint64_t blockGasLimit = yodyDGP.getBlockGasLimit(::ChainActive().Tip()->nHeight + 1);
        size_t count = 0;
        ...
        YodyTxConverter converter(tx, NULL, NULL, contractflags);
        ...
        for(const YodyTransaction& yodyTransaction : yodyTransactions){
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, CBlockIndex* pblockindex, const dev::Address& sender, uint64_t gasLimit, CAmount nAmount) {
    ...
    YodyDGP yodyDGP(globalState.get(), fGettingValuesDGP);
    uint64_t blockGasLimit = yodyDGP.getBlockGasLimit(pblockindex->nHeight + 1);
    ...
    {
        callTransaction = YodyTransaction(nAmount, 1, dev::u256(gasLimit), opcode, dev::u256(0));
    }
    else
    {
        callTransaction = YodyTransaction(nAmount, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    }
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<YodyTransaction>(1, callTransaction), blockGasLimit, pblockindex);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path yodyDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
    ...
    for(YodyTransaction& tx : txs){
        ...
        if(!tx.isCreation() && !globalState->addressInUse(tx.receiveAddress())){
            ...
            result.push_back(ResultExecute{
                execRes,
                YodyTransactionReceipt(dev::h256(), dev::h256(), dev::u256(), dev::eth::LogEntries(), {}, {}),
                CTransaction()
            });
            continue;
        }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // yody
    YodyDGP yodyDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setYodySchedule(yodyDGP.getGasSchedule(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
    uint32_t sizeBlockDGP = yodyDGP.getBlockSize(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t minGasPrice = yodyDGP.getMinGasPrice(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t blockGasLimit = yodyDGP.getBlockGasLimit(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
...
///////////////////////////////////////////////////////////////////////////////////////// yody
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            YodyTxConverter convert(tx, &view, &block.vtx, contractflags);
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertYodyTX.first, blockGasLimit, pindex->pprev);
...
CAmount GetTxGasFee(const CMutableTransaction& _tx)
{
    ...
    if(tx.HasCreateOrCall())
    {
        ...
        YodyTxConverter convert(tx, NULL, NULL, contractflags);
