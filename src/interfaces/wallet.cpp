...
public:
    ...
    DelegationInfo getDelegationContract(const std::string &sHash, bool& validated, bool& contractRet) override
    {
        ...
        if (mi != m_wallet->mapDelegation.end()) {
            ...
            if(keyID)
            {
                ...
                contractRet = m_yodyDelegation.ExistDelegationContract() ? m_yodyDelegation.GetDelegation(address, delegation) : false;
                if(contractRet)
                {
    ...
    DelegationDetails getDelegationDetails(const std::string &sAddress) override
    {
        ...
        {
            uint160 address(*keyID);
            details.c_contract_return = m_yodyDelegation.ExistDelegationContract() ? m_yodyDelegation.GetDelegation(address, delegation) : false;
