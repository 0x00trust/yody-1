...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    YodyTxConverter convert(iter->GetTx(), NULL, &pblock->vtx, contractflags);
    ...
    ByteCodeExec exec(*pblock, yodyTransactions, hardBlockGasLimit, ::ChainActive().Tip());
...
public:
    ...
    void Update(int32_t nHeight)
    {
        ...
        if(nHeight <= checkpointSpan)
        {
            ...
            yodyDelegations.FilterDelegationEvents(events, *this);
        ...
        else
        {
            ...
            {
                std::vector<DelegationEvent> events;
                yodyDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
            ...
            std::vector<DelegationEvent> events;
            yodyDelegations.FilterDelegationEvents(events, *this, cacheHeight + 1);
        ...
        else
        {
            ...
            if(cacheHeight != nHeight)
            {
                ...
                for(auto item: mapAddress)
                {
                    ...
                    if(yodyDelegations.GetDelegation(address, delegation) && YodyDelegation::VerifyDelegation(address, delegation))
                    {
...
void StakeYodys(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{
