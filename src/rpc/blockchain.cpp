...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* tip, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.pushKV("hashStateRoot", block.hashStateRoot.GetHex()); // yody
    result.pushKV("hashUTXORoot", block.hashUTXORoot.GetHex()); // yody
...
static std::vector<RPCResult> MempoolEntryDescription() { return {
    ...
    RPCResult{RPCResult::Type::NUM, "size", "(DEPRECATED) same as vsize. Only returned if yodyd is started with -deprecatedrpc=size\n"
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() >= 3){
        CTxDestination yodySenderAddress = DecodeDestination(request.params[2].get_str());
        if (IsValidDestination(yodySenderAddress)) {
            const PKHash *keyid = boost::get<PKHash>(&yodySenderAddress);
...
UniValue getdelegationinfoforaddress(const JSONRPCRequest& request)
{
    ...
    if (!yodyDelegation.GetDelegation(address, delegation)) {
...
UniValue getdelegationsforstaker(const JSONRPCRequest& request)
{
    ...
    if (!yodyDelegation.FilterDelegationEvents(events, filter)) {
                        ...
                        {RPCResult::Type::OBJ, "scriptPubKey", "",
                            {
                                ...
                                {RPCResult::Type::ARR, "addresses", "array of yody addresses",
